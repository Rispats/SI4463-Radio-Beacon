...
void loop() {
  ...
  if (rxBuffer[0] == 0xA1 || rxBuffer[0] == 0xA2 || rxBuffer[0] == 0xA3) {
    ...
    // Now parse the payload string: "$TX_Ax,dd/mm/yyyy,hh:mm:ss,lat,long*[calc|expec]"
    if (payloadStr[0] == PACKET_START) {
      ...
      if (stopPos > 0) {
        ...
        // Find checksum brackets [calc]
        int bracketStart = -1;
        int bracketEnd = -1;
        
        for (i = stopPos; i < payloadLen; i++) {
          if (payloadStr[i] == '[') bracketStart = i;
          else if (payloadStr[i] == ']') { bracketEnd = i; break; }
        }
        
        if (bracketStart > 0 &&  bracketEnd > bracketStart && (bracketEnd-bracketStart) == 3) 
          ...
          // Verify checksum by computing on content WITHOUT $ and * (for checksum calculation)
          memcpy(contentForChecksum, &payloadStr[1], stopPos - 1);
          contentForChecksum[stopPos - 1] = '\0';
          computeChecksumHex(contentForChecksum, strlen(contentForChecksum), calculatedChecksum);
          
           isValid = (strcmp(calculatedChecksum, receivedChecksum) == 0);
          
          // Build the full message WITH $ and * included in content
          // Format: "$content*[calc|expec][status: VALID/INVALID]" where content already has $content*
          snprintf(fullMessage, sizeof(fullMessage), "%s[%s|%s][status: %s]", 
                   content, calculatedChecksum, receivedChecksum, 
                   isValid ? "VALID" : "INVALID");
          
         
          if (isValid==1) Serial.println(fullMessage);
        } 
        }
      } 
